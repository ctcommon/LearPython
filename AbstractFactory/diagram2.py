...
'''
此改进版本将所有接口设置为一致，不同于第一个版本中，每个工厂都会实现自己的接口，即使接
口是一样的，在本例中不同的是，只在基类当中实现了make_diagram、make_rectangle、以及make_text
通过@classmethod将这三个方法表示为类方法，这样以派生类为参数调用方法的时候，会直接继承基类的方法
然而由于是派生类，Class会使派生类，所以最终调用的是派生类当中的实现版本。
即在继承体系当中，只要其方法在类中有实现，那么不同的类会通过同样的接口调用其本身对应的方法实现。
'''
...
class SvgDiagramFactory(DiagramFactory):
	...
	class Diagram:
		def __init__(self,width,height):
			...
			pxheight = width* SvgDiagramFactory.SVG_SCALE
