...
class Register_application(Tkinter.Frame):       #注册界面
	...
	def pwValue(self,pw): #密码都至少为6位且必须为字母数字
		if len(pw) >= 6:
			for i in pw:
	...
	def nameValue(self,name): #用户名和房间号必须为字母数字
		for i in name:
			if not (i.isalnum() or i == '_'):
				return False
		return True
	def summit(self):
		...
		if not self.nameValue(self.id_input.get()):
			tkMessageBox.showerror('错误', '用户名必须为字母数字')
			return
		if not self.pwValue(self.passwd_input.get()):
			tkMessageBox.showerror('错误', '密码必须大于6位且为字母数字')
			return
		try:
